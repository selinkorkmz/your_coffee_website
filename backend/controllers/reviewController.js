...
const submitComment = async (productId, { userId, comment }) => {
    return new Promise((resolve, reject) => {
        const query = `
            INSERT INTO Ratings (user_id, product_id, comment, approved)
            VALUES (?, ?, ?, 0);
        `;
        db.run(query, [userId, productId, comment], function (err) {
            if (err) {
